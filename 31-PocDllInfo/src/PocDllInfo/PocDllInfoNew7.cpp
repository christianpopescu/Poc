...
BOOL CALLBACK EnumNamesCallback(HMODULE hModule, LPWSTR lpType, LPWSTR lpName, LONG_PTR lParam) {
    std::wcout << L"  Resource Name: " << lpName << std::endl;
    return TRUE;
}

BOOL WINAPI EnumTypesCallback(HMODULE hModule, LPWSTR lpType, LONG_PTR lParam) {
